...
class Pegawai extends CI_Controller {
    ...
    public function export2($id)
    {
        ...
        $data = $this->db->get_where('unit', array('id_unit' => $id))->result();
        ...
        $excel->getActiveSheet()->mergeCells('A1:O2');
        $excel->getActiveSheet()->setCellValue('A1','DAFTAR PEGAWAI UNIT '.$data[0]->nama_unit);
        $excel->getActiveSheet()->getStyle('A1')->applyFromArray($style_row2);
        ...
        // Panggil function view yang ada di SiswaModel untuk menampilkan semua data siswanya
        $siswa = $this->db->get_where('pegawaiview', array('id_unit' => $id))->result();
        $no = 1; // Untuk penomoran tabel, di awal set dengan 1
        $numrow = 4; // Set baris pertama untuk isi tabel adalah baris ke 4
        foreach($siswa as $data)
        { 
